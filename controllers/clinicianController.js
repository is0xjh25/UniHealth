            ...
            clinician.patients.map(async (p) => {
                ...
                if (patient) {
                    const record = await DailyRecord.findOne({
                        $and: [{ _patientID: patient._id }, { date: today }],
                    }).lean()
...
const patientInfo = async (req, res) => {
    try {
        ...
        return res.render('clinician-patient-info', {
            date: today,
            patient: patient,
            record: record,
        })
...
const note = async (req, res) => {
    try {
        ...
        clinician.notes.map((n) => {
            if (n.patient._id.toString() === patient._id.toString())
                notes.push(n)
        })
