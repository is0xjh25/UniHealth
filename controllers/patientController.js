...
const dashboard = async (req, res) => {
    try {
        ...
        if (new Date(req.query.date).isValid())
            today = new Date(req.query.date).toDateString()
        const record = await DailyRecord.findOne({
            $and: [{ _patientID: patientID }, { date: today }],
        }).lean()
        const badge = false
        let difference = new Date().getTime() - patient.registered.getTime()
        let totalDays = Math.ceil(difference / (1000 * 3600 * 24)) + 1
        // if (totalDays === 0) totalDays = 1
        let records = await DailyRecord.find({
            $and: [{ _patientID: patientID }],
        }).lean()
    ...
    try {
        ...
        await Promise.all(
            patients.map(async (p) => {
                ...
                // if (totalDays === 0) totalDays = 1
                let records = await DailyRecord.find({
                    $and: [{ _patientID: p._id }],
                }).lean()
                let rate =
                    Math.round((records.length / totalDays) * 10000) / 100
                let info = {
                    _patientID: p._id,
                    username: p.username,
                    record: rate,
...
const statistics = async (req, res) => {
    try {
        ...
        return res.render('patient-statistics', {
            patient: patient,
            records: records,
            endDate: today,
            startDate: startDate,
